/**
 * Pure camera-decision logic for map screens (promoted from customer shell in UC-019 WI-1).
 *
 ...
 * `useMap()` consumer that APPLIES an intent lives inside each lazy map background
 * (CustomerMapBackground / DriverMapInner), mirroring board/MapPanel's MapCenterController.
 *
 * Promoted to `src/shared/map` (from `features/customer/shell`) so the driver map can
 * consume it without a cross-feature import (rules/web-architecture.md#feature-folders).
 * The old customer path remains as a one-line re-export shim (the priceQuote → shared/pricing
 * pattern) so all existing customer consumers resolve unchanged.
 */
