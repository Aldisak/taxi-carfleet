...
describe('applyFleetBranding', () => {
  it('overrides the primary color from the fleet response', () => {
    const branded = applyFleetBranding(theme, { name: 'Acme', phone: '+420123456789', primaryColorHex: '#ff8800', currency: 'CZK', timeZone: 'Europe/Prague' })
  ...
  it('falls back to the theme primary token when primaryColorHex is null', () => {
    const branded = applyFleetBranding(theme, { name: 'Acme', phone: '+420123456789', primaryColorHex: null, currency: 'CZK', timeZone: 'Europe/Prague' })
  ...
  it('ignores a malformed (non #RRGGBB) color and keeps the theme token', () => {
    const branded = applyFleetBranding(theme, { name: 'Acme', phone: '+420123456789', primaryColorHex: 'red', currency: 'CZK', timeZone: 'Europe/Prague' })
    expect(branded.colors.primary).toBe(theme.colors.primary)
  })
})
