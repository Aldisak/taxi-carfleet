...
### UC-007 (reports, audit, tenant onboarding)

1. **Reports are SQL-aggregated, never ToList-then-group (AC#2).** The driver report (`GET /reports/drivers`) and fleet report (`GET /reports/fleet`) compute all counts/sums/averages server-side. Per-Prague-day bucketing uses `(created_at AT TIME ZONE 'Europe/Prague')::date` as the GROUP BY key inside an EF Core `Database.SqlQuery<T>` (the Npgsql provider here has no `EF.Functions.AtTimeZone`). Hours-online is a separate, window-clamped `driver_shifts` query merged in memory — deliberately NOT folded into the orders GROUP BY. FromSql result columns MUST be aliased in snake_case (`AS "card_czk"`) because EF maps them by the property's configured (snake_case) name — see CLAUDE.md.
2. **50k perf seed is a separate dev/test script, not the demo seeder.** `Infrastructure/Seed/ReportSeedScript` bulk-inserts N completed orders for one fleet via a single `INSERT … SELECT FROM generate_series` (one round-trip; row-by-row EF would distort the measurement). It is idempotent (skips if the fleet already has orders) and is invoked only by the perf test (`FleetReport_50kOrders_RespondsUnder300ms`, tagged `[Trait("Category","Perf")]`). The normal test suite runs with `--filter "Category!=Perf"` so 50k is never seeded on the hot path. Backed by the A1 `(fleet_id, created_at)` / `(fleet_id, status, created_at)` composite indexes.
3. **Driver-position history retention deferred (not in v1).** The retention job does not prune driver positions because v1 does not persist a driver-position history table (positions are live-only via SignalR + the in-memory store). The assignment's "delete driver positions older than 7 days" clause activates only if/when a position-history table is added.

### UC-008 (infra, CI/CD & operations)

1. **Runtime base image `aspnet:10.0`, not `aspnet:9.0` (deviation from assignment 08 §1).** Assignment 08's Dockerfile example specifies `mcr.microsoft.com/dotnet/aspnet:9.0`. The project targets `net10.0` (`api/src/Taxi.Api/Taxi.Api.csproj`), so the runtime must be `mcr.microsoft.com/dotnet/aspnet:10.0`; a 9.0 runtime cannot run a net10.0 assembly. The SDK build stage is `dotnet/sdk:10.0`. Documented in `api/Dockerfile`.

2. **Image-size target <120 MB is aspirational on the Debian base.** `aspnet:10.0` (Debian) is ~220 MB before the app; plus the `curl` install for the HEALTHCHECK. The <120 MB assignment target is not reachable framework-dependent on Debian. The honest path (documented follow-up) is the alpine runtime base (`aspnet:10.0-alpine` + `apk add curl`), which the SDK-alpine build stage would match. Kept on Debian for v1 to match the existing tested build; size is operational-deferred per the spec's Verifiability Reality.

3. **Edge Caddy needs a custom xcaddy build; the web container's Caddy does not.** Stock `caddy:2-alpine` has neither the DNS-challenge TLS plugins (`caddy-dns/cloudflare`, `caddy-dns/hetzner`) nor the `rate_limit` module (`mholt/caddy-ratelimit`). The production edge proxy (`infra/Caddyfile`) uses both, so it is built from `infra/caddy/Dockerfile` (xcaddy). Consequence: `caddy validate` against a stock binary fails on `acme_dns`/`rate_limit` — validation is deferred to the xcaddy image. The inner web-image Caddyfile (`infra/web/Caddyfile`) only does static serve + `/api`+`/hubs` proxy and runs fine on stock `caddy:2-alpine`.

4. **Wildcard DNS-01 TLS is primary; on-demand TLS is the documented fallback.** `*.{$DOMAIN}` needs a DNS-01 challenge (HTTP-01 cannot issue wildcards), so one wildcard cert covers every fleet subdomain with no per-fleet deploy. The `infra/Caddyfile` documents the Cloudflare and Hetzner `acme_dns` variants and, as a fallback where a DNS API token is impractical, an on-demand-TLS block (per-subdomain HTTP-01 issuance gated by an `ask` endpoint — trade-off: first-hit latency + Let's Encrypt rate limits).

5. **Backups use rclone to S3-compatible storage; restore.sh treats the SELECT sanity check as authoritative.** The `backup` container (`infra/backup/`) is `postgres:16-alpine` (matching the DB major so `pg_dump` never version-refuses) + rclone. `infra/restore.sh` intentionally does NOT pass `--exit-on-error` to `pg_restore` and does not fail on pg_restore's exit code — a cross-minor-version restore emits benign warnings (e.g. a newer client's `SET transaction_timeout`). The real pass/fail is `SELECT count(*) FROM orders` succeeding after restore. Verified locally round-tripping a `pg_dump -Fc | gzip` into a throwaway postgres:16 container.
...
## 2026-09-15 — UC-011 Multilingual UI: six locales, full culture codes, auto-detect → Czech fallback

The dispatcher/driver/customer web app ships a runtime UI-language switcher. Decisions:

- **Six supported languages:** Czech (`cs-CZ`, default), English (`en-US`, i18next fallback), Russian (`ru-RU`), Ukrainian (`uk-UA`), Filipino (`fil-PH`), German (`de-DE`). Russian/Ukrainian/Filipino/German reflect the driver+customer demographics of the target fleets; Czech and English are the operator baseline.
- **Full culture codes, not bare primary subtags.** The registry keys and locale JSON filenames use `xx-YY` culture codes (`cs-CZ`, not `cs`) so region-specific variants remain addressable later without a rename ripple. `SUPPORTED_LOCALES` in `web/src/shared/i18n/locales.ts` is the single source of truth; the selector, i18n config, browser detection (`resolveInitialLanguage.ts`), and the parity test all derive from it — no module hardcodes a language by code literal (enforced by `extensibility.test.ts`, AC#8).
- **Auto-detect on first visit → Czech fallback.** `applyInitialLanguage()` (called once from `main.tsx`) resolves `navigator.languages` + any stored choice: a valid stored value wins, else exact culture-code match, else primary-subtag match (subtag→code map derived from the registry; only the legacy `tl → fil-PH` Tagalog alias is hardcoded), else `DEFAULT_LOCALE` (`cs-CZ`). The chosen language persists in `localStorage` (`app.language`) and sets `<html lang>`. Playwright pins `locale: 'cs-CZ'` so existing e2e specs stay Czech.
- **Formatting is a localization non-goal (spec §21).** Money stays `cs-CZ`/`… Kč` and dates stay `Europe/Prague` in every UI language. The ~30 `cs-CZ` formatting literals (`money.ts` `Intl.NumberFormat('cs-CZ')`, TrackingPage, AuditPage, …) are correct and exempt from the extensibility scan; `formatStaysCs.test.ts` guards that switching UI language does not change money/date formatting.
- **Static-JSON extensibility contract (AC#8).** Adding a 7th language is exactly three edits: drop in one `xx-YY.json` (key-complete copy of `cs-CZ.json`), add one `SUPPORTED_LOCALES` entry, add one import + resources line in `index.ts`. No edit to the selector, parity test, or `resolveInitialLanguage.ts` is required — all three are registry-driven. `extensibility.test.ts` scans non-test production source and fails if any of the five non-default culture codes leaks outside the registry whitelist (`locales.ts`, `index.ts`, `sentinelKeys.ts`, `resolveInitialLanguage.ts`, `shared/date/analyticsRange.ts`).
- **Follow-up: native-speaker review of the four machine translations.** `ru-RU`, `uk-UA`, `fil-PH`, `de-DE` were produced without a native reviewer. The `sentinelKeys.ts` gate proves each locale was actually translated (sentinel keys differ from Czech) but NOT that it is native-quality. A native-speaker copy review of these four locales (register split: formal customer / informal driver — German `Sie`/`du`, Filipino `po/opo`) is a recommended follow-up before a production launch in those markets.


---

## 2026-09-15 — UC-012 WI-4: SuperAdmin `/admin` screens get their own size-limit bucket

The SuperAdmin per-tenant settings editor (`AdminTenantSettingsPage`) is a required 20-field form. It adds ~3.1 KB brotli of irreducible field/styled-component code. The `web/package.json` `size-limit` "app js" budget was already at 257.88/260 KB (99.2%) before this WI, so the new page pushed it 987 B over 260 KB.

**Root cause of the miss:** the UC-012 designer assumed the page would "ride the existing lazy admin chunk (size unaffected)." That was wrong twice: (1) there is no single admin chunk — each admin screen was its own entry-named lazy chunk; (2) the "app js" size-limit glob (`dist/assets/*.js` minus `analytics-*.js`) *sums all* non-analytics chunk files, so any new lazy chunk counts against the main budget regardless of code-splitting.

**Decision: give `/admin` its own size bucket, mirroring analytics.** `web/vite.config.ts` `manualChunks` now collapses everything under `features/admin/` into one named `admin` chunk, and `web/package.json` excludes `admin-*.js` from the main "app js" bucket and adds a dedicated `admin chunk (brotli, SuperAdmin-only)` budget (70 KB). Collapsing all five `/admin` screens (fleets, tenant settings, login, guard, platform) plus their form/validation modules into one chunk moved ~61 KB brotli of SuperAdmin-only weight *out* of the main bundle (257.88 KB → 196.3 KB) and into the new admin chunk (63.96 KB, no chart.js/leaflet — the platform sparkline is a pure SVG-path module). The admin budget is set at 70 KB to leave headroom.

**Rationale:** the `/admin` screens are SuperAdmin-only cross-tenant ops — they are never fetched by a normal dispatcher/driver/customer session, exactly the same reason the analytics chunk is already excluded from the main budget. Isolating their weight keeps the main app-js budget honest for the traffic that actually ships to end users, and gives the admin surface its own headroom to grow. This was chosen over simply raising the 260 KB limit (which would have quietly absorbed SuperAdmin-only weight into the number that guards the end-user bundle). `AskUserQuestion` was unavailable in the developer subagent, so this judgment was made per the "note and proceed with best judgment" fallback and the WI note's explicit "investigate chunking before touching budgets" instruction.

## 2026-09-24 — UC-021 WI-9: admin chunk budget 70 → 76 KB (dispatcher/admin desk-kit redesign)

The UC-021 redesign restyled the SuperAdmin screens (`AdminFleetsPage`, `AdminTenantSettingsPage`, `PlatformScreen`, `AdminLoginPage`, `AdminGuard`) onto the new `shared/ui/desk/` kit — richer markup (per-fleet `Table` + sparkline column, `Stat` KPI grid, 4-panel tenant form with a live colour-contrast rule, one-time-password copy flow). That pushed the SuperAdmin-only `admin` chunk from 63.96 KB → 72.19 KB brotli, 2.19 KB over the 70 KB budget set on 2026-09-15.

**Decision: raise the `admin` budget to 76 KB.** The desk kit itself is shared (imported by the eager board/orders chunks too), so it is NOT duplicated into `admin` — the growth is the admin screens' own richer UI. The `admin` chunk is SuperAdmin-only cross-tenant ops, never fetched by a normal dispatcher/driver/customer session (same rationale as the analytics exclusion), so this weight does not affect the end-user bundle. The end-user "app js" budget (260 KB) is untouched and comfortable at 203.87 KB. 76 KB leaves ~5% headroom above the current 72.19 KB. Trimming 2.19 KB would mean removing legitimate SuperAdmin features from a chunk that never ships to end users — not worth it.
