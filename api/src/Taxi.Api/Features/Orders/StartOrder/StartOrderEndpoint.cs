...
internal sealed class StartOrderEndpoint(TaxiDbContext dbContext, OrderService orderService, TimeProvider timeProvider)
    ...
    public override async Task HandleAsync(StartOrderRequest req, CancellationToken ct)
    {
        await IdempotencyHelper.HandleAsync(HttpContext, dbContext, timeProvider, req,
            async (storeAsync, ct2) =>
            {
                ...
                var result = await orderService.TransitionAsync(req.Id, OrderTransition.Start, actor, null, ct2);
